...
export class AppHttpService{
    ...
    builder (resource: string){
        this.url = 'http://api.matheus.bhz.br/api/v1/' + resource;
        return this;
    }
    ...
    protected toPromise(request) {
        return request.toPromise()
            ...
            .catch((err) => {
                ...
                if(err.status === 422) {
                    message = 'falha de validade, verifique os campos';
                }

                if(err.status === 404) {
                    message = 'Impossivel se conectar ao servidor, verifique sua conexão ou tente novamente em alguns minutos';
                }
